...
Public Type Micrometer
    lngHandle As Long
    strDescription As String
    strSerialNumber As String
    strValue As String
    dblValue As Double
    open As Boolean
End Type

Global lngMicroHandle As Long

Global numberOfMicrometers As Long
Global micrometers() As Micrometer
Global currentMicrometer As Long

Public Sub loadMicrometers()
    Dim i As Long
    Dim strDesc As String * 256
    Dim strSerial As String * 256
    Dim ftStatus As Long
    
    If FT_GetNumDevices(numberOfMicrometers, vbNullString, FT_LIST_BY_NUMBER_ONLY) <> FT_OK Then
        numberOfMicrometers = 0
    Else
        ReDim micrometers(numberOfMicrometers)
        For i = 0 To numberOfMicrometers - 1
            If FT_ListDevices(i, strDesc, FT_LIST_BY_INDEX Or FT_OPEN_BY_DESCRIPTION) = FT_OK Then
                micrometers(i).strDescription = Trim(strDesc)
            End If
            
            If FT_ListDevices(i, strSerial, FT_LIST_BY_INDEX Or FT_OPEN_BY_SERIAL_NUMBER) = FT_OK Then
                micrometers(i).strSerialNumber = Trim(strSerial)
            End If
            
            If FT_OpenEx(strDesc, FT_OPEN_BY_DESCRIPTION, micrometers(i).lngHandle) = FT_OK Then
                micrometers(i).open = True
                ftStatus = FT_SetBaudRate(micrometers(i).lngHandle, 4800)
                ftStatus = FT_SetDataCharacteristics(micrometers(i).lngHandle, FT_BITS_7, FT_STOP_BITS_2, FT_PARITY_EVEN)
                setMicrometerTimeout i, 1000
'                ftStatus = FT_SetTimeouts(micrometers(i).lngHandle, 2000, 0)
            Else
                micrometers(i).open = False
            End If
        Next i
    End If
End Sub

Public Sub setMicrometerTimeout(Index As Long, timeout As Long)
    Dim ftStatus As Long
    
    ftStatus = FT_SetTimeouts(micrometers(Index).lngHandle, timeout, 0)
End Sub

Public Sub readAllMicrometers()
    Dim i As Long
...
Public Sub readMicrometer(Index As Long)
    Dim readStr As String * 9
    Dim bytesRead As Long
    Dim ftStatus As Long
    
    If Index >= 0 And Index < numberOfMicrometers Then
        If micrometers(Index).open Then
            ftStatus = FT_Read(micrometers(Index).lngHandle, readStr, 9, bytesRead)
            If ftStatus = FT_OK Then
                If bytesRead > 0 Then
                    micrometers(Index).dblValue = val(readStr)
                    micrometers(Index).strValue = Trim(readStr)
                End If
            End If
        End If
    End If
...
Public Sub closeAllMicrometers()
    Dim i As Integer
...
Public Sub closeMicrometer(Index As Integer)
    Dim ftStatus As Long
    
    ftStatus = FT_Close(micrometers(Index).lngHandle)
End Sub
